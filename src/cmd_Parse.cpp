...
void cmd13(cJSON *root) // 读取服务器设置
{
    ...
    EEPROM.writeString(150, WiFi_Data.serverip.c_str());    //在150地址处写一串字符串, 保险起见留40个字节的余量 
    EEPROM.writeInt(191, WiFi_Data.serverport);     //在191字节处写一个int型变量,存放服务器端口
...
void cmd19(cJSON *root) // 初始化设备信息
{
    ...
    EEPROM.writeString(100, ProjectData.device_ID.c_str()); //在100地址处写设备ID字符串
    EEPROM.commit(); // 需要提交才能正真地把数据写入EEPROM
}